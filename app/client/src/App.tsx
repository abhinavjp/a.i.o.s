      ...
      <main className="dashboard" id="command">
        ...
        <div className="content-grid">
          <div className="main-column">
            ...
            <section className="panel"><div className="panel-heading"><div><span className="eyebrow">Operator queue</span><h2>Asks ({dashboard.asks.length})</h2></div></div>{dashboard.asks.length === 0 ? <p className="panel-note subtle">No pending asks.</p> : <div className="proof-list">{dashboard.asks.map((ask) => <div className="proof-row" key={ask.id}><strong>{ask.kind}</strong><small>{ask.workItemId ?? "No work item"} · waiting since {ask.createdAt}</small></div>)}</div>}</section>
