      ...
      <main className="dashboard" id="command">
        ...
        <div className="content-grid">
          <div className="main-column">
            ...
            <section className="panel"><div className="panel-heading"><div><span className="eyebrow">Operator queue</span><h2>Asks ({dashboard.asks.length})</h2></div>{dashboard.asks.length > 0 && <button type="button" onClick={() => setCatchUpIndex(0)}>Catch up</button>}</div>{dashboard.asks.length === 0 ? <p className="panel-note subtle">No pending asks.</p> : <div className="proof-list">{dashboard.asks.map((ask) => <div className="proof-row" key={ask.id}><div><strong>{ask.kind}</strong><small>{ask.workItemId ?? "No work item"} · waiting since {ask.createdAt}</small></div><button type="button" onClick={() => void decideAsk(ask.id, "approved")}>Approve</button><button type="button" onClick={() => void decideAsk(ask.id, "declined")}>Decline</button></div>)}</div>}</section>{catchUpIndex !== null && dashboard.asks[catchUpIndex] && <div role="dialog" aria-label="Catch-up mode" className="catch-up"><h2>{dashboard.asks[catchUpIndex].kind}</h2><p>{catchUpIndex + 1} of {dashboard.asks.length} asks remaining</p><p>{dashboard.asks[catchUpIndex].workItemId ?? "No work item"}</p><button onClick={() => void decideAsk(dashboard.asks[catchUpIndex].id, "approved")}>Approve (A)</button><button onClick={() => void decideAsk(dashboard.asks[catchUpIndex].id, "declined")}>Decline (D)</button><button onClick={() => setCatchUpIndex((catchUpIndex + 1) % dashboard.asks.length)}>Skip (S)</button><button onClick={() => setCatchUpIndex(null)}>Leave (Esc)</button></div>}
