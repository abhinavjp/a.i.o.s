...
export function App() {
  ...
  if (view === "advanced") return <AdvancedControls manualPaused={dashboard.controls.manualPaused} backLabel={advancedReturnToSetup ? "Back to setup" : "Back to Mission Control"} onBack={() => { setView("command"); if (advancedReturnToSetup) { setSetupDismissed(false); setAdvancedReturnToSetup(false); } }} onPause={() => { void togglePause(); }}>
    ...
    <section className="mc-advanced-section mc-section" id="mc-advanced-agents" aria-label="Agents and tasks">
      <section className="panel task-panel" aria-label="Direct task execution">
        <div className="panel-heading"><div><span className="eyebrow">Direct task</span><h2>Ask the coordinator</h2></div><span className="quiet-tag">{runningVersion ? `Sarathi ${runningVersion}` : "version unknown"}</span></div>
        <form className="task-form" onSubmit={handleSubmit}>
          <div className="task-routing-fields"><label htmlFor="task-engine">Engine<select id="task-engine" aria-label="Task engine" value={taskEngine} onChange={(event) => setTaskEngine(event.target.value as "inherit" | AgentEngineKind)}><option value="inherit">Inherit</option><option value="hermes">Hermes</option><option value="codex">Codex</option><option value="claude-code">Claude Code</option></select></label>{taskEngine !== "inherit" && <><label htmlFor="task-configuration">Configuration<input id="task-configuration" value={taskConfiguration} onChange={(event) => setTaskConfiguration(event.target.value)} /></label><label htmlFor="task-model">Model<input id="task-model" value={taskModel} onChange={(event) => setTaskModel(event.target.value)} /></label></>}</div>
          <label htmlFor="task-input">Task <span>· what should move next?</span></label>
          <div className="task-input-row"><input id="task-input" value={task} onChange={(event) => setTask(event.target.value)} placeholder="Describe the work" /><button type="submit">Run task <span>↗</span></button></div>
          <div className="task-routing"><label htmlFor="task-specialist">Specialist ID<input id="task-specialist" value={taskRouting.specialistId} onChange={(event) => setTaskRouting((draft) => ({ ...draft, specialistId: event.target.value }))} placeholder="optional specialist id" /></label><label htmlFor="task-workflow">Workflow ID<input id="task-workflow" value={taskRouting.workflowId} onChange={(event) => setTaskRouting((draft) => ({ ...draft, workflowId: event.target.value }))} placeholder="optional workflow id" /></label><label><input type="checkbox" checked={taskRouting.overridePrimary} onChange={(event) => setTaskRouting((draft) => ({ ...draft, overridePrimary: event.target.checked }))} /> Override task primary</label>{taskRouting.overridePrimary && <label htmlFor="task-primary">Primary override model<input id="task-primary" value={taskRouting.primaryModel} onChange={(event) => setTaskRouting((draft) => ({ ...draft, primaryModel: event.target.value }))} required /></label>}<label><input type="checkbox" checked={taskRouting.overrideFallback} onChange={(event) => setTaskRouting((draft) => ({ ...draft, overrideFallback: event.target.checked }))} /> Override task fallback chain</label>{taskRouting.overrideFallback && <label htmlFor="task-fallback">Fallback override model (blank clears)<input id="task-fallback" value={taskRouting.fallbackModel} onChange={(event) => setTaskRouting((draft) => ({ ...draft, fallbackModel: event.target.value }))} /></label>}</div>
        </form>
        {admission?.plan && <div className="admission-summary">Resolved: <strong>{admission.plan.primary.engine}</strong>{admission.executedEngineRoute && <span> · executed {admission.executedEngineRoute.engine}/{admission.executedEngineRoute.configuration}</span>} · readiness {admission.readiness?.state ?? "unmeasured"}</div>}
        {status === "running" && <button type="button" onClick={() => void stopTask()} disabled={isStopping}>{isStopping ? "Stopping…" : "Stop task"}</button>}
        {activeStall && activeStall.state !== "active" && <p role="status">Task stalled: {activeStall.state} threshold passed.</p>}
        {executionMessage && <p role="alert">{executionMessage}</p>}{output.length > 0 && <pre className="task-output">{output.join("\n")}</pre>}
        {status !== "idle" && <div className={`task-status ${status}`}><span className="status-dot" /> Run status: {statusLabel(status)}</div>}
      </section>
